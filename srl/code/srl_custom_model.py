...
class CustomSemanticRoleLabeler(Model):
    """
    ...
    def __init__(self, vocab: Vocabulary,
        ...
        super(CustomSemanticRoleLabeler, self).__init__(vocab, regularizer)
        ...
        self.span_metric = CustomSpanBasedF1Measure(vocab, tag_namespace="labels", ignore_classes=["V"], label_encoding=self.label_encoding)
    ...
    def forward(self,  # type: ignore
                ...
                span_matrix=None,
                eval_metric=True) -> Dict[str, torch.Tensor]:
...
def convert_bio_tags_to_conll_format(labels: List[str]):
    ...
    for i, label in enumerate(labels):
        ...
        if (label[0] in ["B", "U"]) or i == 0 or label[1:] != labels[i - 1][1:]:
        ...
        if i == sentence_length - 1 or (labels[i + 1][0] in ["B", "U"]) or label[1:] != labels[i + 1][1:]:
            new_label = new_label + ")"
