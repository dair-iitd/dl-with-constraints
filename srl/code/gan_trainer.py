...
def get_mixer(it1, ds1, it2, ds2, num_gpus=1,id1 = 0, which_mixer='bm',min_pct_of_ds2 = 0.0,shuffle=True):
    #Pdb().set_trace()
...
class SemiSupervisedTrainer:
    def __init__(self, trainer, constraints_model, dd_optimizer, big_iterator, unlabelled_dataset=None, 
                semi_supervision=False,  which_mixer = 'cm', dd_warmup_iters=100, dd_update_freq=2, constraints_wt=0, 
                calc_valid_freq=1, backprop_after_xbatches = 1, min_pct_of_unlabelled = 0.0, dd_semi_warmup_iters=0.0,
                dd_increase_freq_after=0,dd_increase_freq_by=0, dd_decay_lr=0, debug = False, chfile=None, shuffle=True, 
                dd_decay_lr_after=1.0, grad_norm_before_warmup = 0):
        ...
        self.dd_decay_lr_after = dd_decay_lr_after
        #assert self.dd_decay_lr_after  > 0
        ...
        self.grad_norm_before_warmup = grad_norm_before_warmup != 0 
        ...
        self.load_constraints_model(chfile)
    ...
    def batch_loss(self, batch_group: List[TensorDict], for_training: bool, eval_metric = True):
        ...
        # TODO - what is regularization_penalty?
        if for_training and eval_metric:
            output_dict['regularization_penalty'] = self.trainer.model.get_regularization_penalty()  

        return output_dict 

    def step(self,loss,should_norm_grad=True):
        self.trainer.optimizer.zero_grad()
        loss.backward()
        batch_grad_norm = None 
        if should_norm_grad:
            #batch_grad_norm = training_util.rescale_gradients(self.trainer.model, self.trainer._grad_norm)
    ...
    def semi_train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        mixed_generator, num_training_batches = get_mixer(self.trainer.iterator, self.trainer.train_data, self.trainer.iterator,  self.unlabelled_dataset,num_gpus, self.labelled_id, self.which_mixer,self.min_pct_of_unlabelled, shuffle=self.shuffle)

        #generator for lambda update
        mixed_generator_for_lambda, _ = get_mixer(self.trainer.iterator, self.trainer.train_data, self.trainer.iterator,  self.unlabelled_dataset, num_gpus, self.labelled_id, 'cm', shuffle=self.shuffle)
        ...
        #Pdb().set_trace()
        fh = None
        self.debug = self.debug and (epoch > 100) and (epoch < 120)
        if self.debug:
            fh = open(os.path.join(self.trainer._serialization_dir,'debug_output_{}.csv'.format(epoch)),'w')
            print('#Word ; Gold Tag ; Decoded Tag ; Top5 Tags ; Top5 Probs ; Penalties',file = fh)
            ind2lab = self.trainer.model.vocab.get_index_to_token_vocabulary(namespace='labels')
        for batch_group,group_id in train_generator_tqdm:
            ...
            if self.constraints_model is not None:                
                curBatch = batch_group[0]                
                penalties = self.constraints_model(output_dict['class_probabilities'], output_dict["mask"], curBatch["span_matrix"],self.debug) 

            
            if self.debug and group_id == self.labelled_id:
                global_count = helper.write_decoded_output_to_file(batch_group[0], output_dict, ind2lab, penalties, global_count,None,fh)
            ...
            if (batches_since_last_step == self.backprop_after_xbatches) or (self.total_supervised_iters < self.dd_semi_warmup_iters):
                #print("STEP THROUGH! : {}. loss: {} agg_loss: {}".format(group_id, loss, agg_loss))
                batch_grad_norm = self.step(agg_loss,(self.grad_norm_before_warmup or (self.total_supervised_iters >= self.dd_warmup_iters)))
                batches_since_last_step = 0
                agg_loss = 0.0
                flag = False 
            else:
            ...
            if  (self.constraints_model is not None) and (self.dd_optimizer is not None) and (self.total_supervised_iters >= self.dd_warmup_iters) and (self.total_supervised_iters - self.last_lambda_update >=  self.dd_update_freq):
                ...
                if (self.dd_increase_freq_after >  0) and (self.count_lambda_updates % self.dd_increase_freq_after == 0):
        ...
        metrics['siter'] = self.total_supervised_iters 
        metrics['liter'] = self.count_lambda_updates  
        metrics['ufreq'] = self.dd_update_freq 
        metrics["uloss"] = float(unlabelled_loss / (batches_this_epoch+unlabelled_batches_this_epoch)) 
        if self.constraints_model is not None:
            for param_group in self.dd_optimizer.param_groups:
                metrics['ddlr'] = param_group['lr']                            
                break
            # 
    ...
    def lambda_update(self, batch_group):
        with torch.no_grad():
            ...
            self.trainer.model.train()
        # TODO VV IMPORTANT - make span matrix, need the batch        
        curBatch = batch_group[0]            
        penalties = self.constraints_model(output_dict['class_probabilities'].detach(), output_dict["mask"].detach(), curBatch["span_matrix"].detach()) 
        self.dd_optimizer.zero_grad()
        loss = -1.0*penalties['loss']
        loss.backward()
        self.dd_optimizer.step()
        self.trainer._tensorboard.log_parameter_and_gradient_statistics(self.constraints_model, None)
        lr_decay_after = self.dd_decay_lr_after
        lambda_iter = self.count_lambda_updates + 1
        #Pdb().set_trace()
        if self.dd_decay_lr == 1:
            #sqrt 
        ...
        elif self.dd_decay_lr == 2:
            #linear
            lr_decay_after = 1.0/lr_decay_after 
            for param_group in self.dd_optimizer.param_groups:
                #param_group['lr'] = param_group['lr']*((self.count_lambda_updates+1)/(self.count_lambda_updates+2))
                param_group['lr'] = param_group['lr']*(float(lr_decay_after*(lambda_iter-1)+1)/float(lr_decay_after*lambda_iter+1))
        elif self.dd_decay_lr == 3:
            #exp
            assert lr_decay_after <= 1.0
            for param_group in self.dd_optimizer.param_groups:
    ...
    def save_constraints_model(self,epoch=-1):
        ...
        chpoint['dd_optim'] = self.dd_optimizer.state_dict() if self.dd_optimizer is not None else None
        chpoint['dd_update_freq'] = self.dd_update_freq
        chpoint['last_lambda_update'] = self.last_lambda_update 
    ...
    def load_constraints_model(self, chfile=None):
        if chfile==None:
        ...
        if os.path.exists(chfile):
            ...
            self.last_lambda_update = cp['last_lambda_update']
