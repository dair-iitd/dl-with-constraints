...
class MILDataset():
    def __init__(self, entity_bags_dict, entity_type_dict, batch_attributes, entities, vocab_dict, embeddings, encoder, batch_size, num_fb_types, transformer, bag_size, entity_dict, cross_wikis, typenet_matrix,train = True):
        ...
        self.entities_list = list(entities)
        ...
        self.typenet_matrix = typenet_matrix 
    ...
    def shuffle(self):
        np.random.shuffle(self.entities_list)

    def get_batch(self,num_epochs = 1):
        ...
        for epoch in range(num_epochs):
            ...
            for ent in self.entities_list:
                ...
                list_items = self.transformer(all_mentions, self.vocab_dict, self.entity_dict, self.cross_wikis, self.embeddings, self.bag_size, self.train, self.encoder_type, self.typenet_matrix)
                bit_vec = [0]*self.num_fb_types  # predictions are made only for freebase types
                if self.entity_type_dict is not None:
...
def MILtransformer(all_mentions, vocab_dict, entity_dict, cross_wikis, embeddings, bag_size, train, encoder_type, typenet_matrix):
    '''
